...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
	...
	*mn = an;
	...
	//Keep only the first match, discard all others.
	for (i = 0; i < an; i++)
	{
		if (seen[m[i].bi])
		{
...
point project_point(matrix H, point p)
{
	...
	matrix p_hom = make_matrix(3, 1);

	c.data[0][0] = p.x;
	c.data[1][0] = p.y;
	c.data[2][0] = 1;

	p_hom = matrix_mult_matrix(H, c);
	// TODO: project point p with homography H.
	// Remember that homogeneous coordinates are equivalent up to scalar.
	// Have to divide by.... something...
	point q = make_point(0, 0);
	q.x 	= p_hom.data[0][0]/p_hom.data[2][0];
	q.y 	= p_hom.data[1][0]/p_hom.data[2][0];
...
int model_inliers(matrix H, match *m, int n, float thresh)
{
	...
	for (i = 0; i < n; i++)
	{
		projection = project_point(H, m[i].p); 
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
	...
	for (i = 0; i < k; i++)
	{
		...
		hom_tmp = compute_homography(m,4);	
		inliers = model_inliers(hom_tmp, m, n, thresh);
...
image combine_images(image a, image b, matrix H)
{
	...
	for(k = 0; k < a.c; ++k)
	{
		for(j = 0; j < a.h; ++j)
		{
			for(i = 0; i < a.w; ++i)
			{
				float pxl = get_pixel(a,i,j,k);
				set_pixel(c, i - dx, j - dy, k, pxl);
			}
		}
	}
	...
	// estimate the vGlue of b at that projection, then fill in image c.
	...
	for(k = 0; k < b.c; ++k)
	{
		for(j = topleft.y; j < botright.y; ++j)
		{
			...
			{
				point p1 = project_point(H, make_point(i,j));
				if (p1.x >= 0 && p1.x < b.w && p1.y >= 0 && p1.y < b.h)
				{
					float bl = bilinear_interpolate(b, p1.x, p1.y, k);
					set_pixel(c, i-dx, j-dy, k, bl);
				}
			}
		}
	}
